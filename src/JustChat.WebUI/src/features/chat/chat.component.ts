...
export class ChatComponent implements AfterViewInit, OnInit, OnDestroy {
  ...
  private readonly senderTooltipPad = 8;
  private readonly senderTooltipCursorOff = 12;
  ...
  protected onSenderAvatarEnter(event: MouseEvent, sender: UserProfileDetails): void {
    ...
    this.positionSenderTooltipNearCursor(event);
  }

  protected onSenderAvatarMove(event: MouseEvent): void {
    if (this.avatarTooltipSender) {
      this.positionSenderTooltipNearCursor(event);
    }
  }
  ...
  private positionSenderTooltipNearCursor(event: MouseEvent): void {
    const vw = window.innerWidth;
    const vh = window.innerHeight;
    const cx = event.clientX;
    const cy = event.clientY;
    const off = this.senderTooltipCursorOff;
    const pad = this.senderTooltipPad;
    const estW = this.senderTooltipEstW;
    const estH = this.senderTooltipEstH;

    // Default: top edge above cursor, box shifted right from cursor
    let left = cx + off;
    let top = cy - off - estH;
